...
old_actions = {"Changed": CHANGED, "Added": ADDED, "Deleted": DELETED}
...
def migrate_changed_data(apps, schema_editor):
    ...
    perc = 0
    for count, ch in enumerate(
        Change.objects.using(schema_editor.connection.alias).all()
    ):
        if count % chunk == 0:
            print("{}%".format(perc))
            perc += 1
        ...
        try:
            for i in comment[1].split(')\n"'):
                ...
                data.append(
                    {
                        "field": field,
                        "values": {"old": _get_val(old), "new": _get_val(new)},
                    }
                )
        except Exception:
    ...
    operations = [
        ...
        migrations.AddField(
            ...
            field=models.TextField(
                blank=True, help_text="The old data of changed fields.", null=True
            ),
        ),
