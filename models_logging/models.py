...
from .settings import ADDED, CHANGED, DELETED, JSON_ENCODER_PATH, LOGGING_USER_MODEL
...
class Revision(models.Model):
    ...
    comment = models.TextField(
        _("comment"), blank=True, help_text=_("A text comment on this revision.")
    )
...
class Change(models.Model):
    ...
    def get_changes_by_obj(model, obj_id, related_objects="__all__"):
        ...
        for rel_model in related_objects:
            if isinstance(rel_model, models.OneToOneRel):
                ...
                values = (
                    getattr(obj, rel_model.get_accessor_name())
                    .annotate(pk_str=Cast("pk", output_field=models.TextField()))
                ...
                values = (
                    getattr(obj, rel_model.get_attname())
                    .annotate(pk_str=Cast("pk", output_field=models.TextField()))
                    .values_list("pk_str", flat=True)
                )
            else:
        ...
        for v in history_objects:
            qobj.add(
                models.Q(
                    content_type_id=v["content_type_id"], object_id__in=v["values"]
                ),
                models.Q.OR,
            )
    ...
    def revert(self):
        with transaction.atomic():
            data = {
                field: values.get("old") for field, values in self.changed_data.items()
            }
