...
class HistoryAdmin(admin.ModelAdmin):
    ...
    def history_view(self, request, object_id, extra_context=None):
        ...
        assert (
            isinstance(self.inline_models_history, (tuple, list))
            or self.inline_models_history == "__all__"
        )
    ...
    def get_changes_queryset(self, obj):
        qs = Change.get_changes_by_obj(
            obj, related_models=self.get_related_objects_for_changes()
        )
...
class ContentTypeFilterForChange(RelatedFieldListFilter):
    def field_choices(self, field, request, model_admin):
        ...
        items = ContentType.objects.get_for_models(
            *models_logging_config.registered_models
        )
...
class ChangeAdmin(FastObjectsCountAdminModel):
    list_display = [
        ...
        "content_type",
        "get_comment",
        "get_link_admin_object",
        "user",
    ]
    list_filter = [
        ("content_type", ContentTypeFilterForChange),
        "date_created",
        "action",
    ]
    ...
    def has_delete_permission(self, request, obj=None):
        return (
            CAN_DELETE_CHANGES(request, obj)
            if callable(CAN_DELETE_CHANGES)
            else CAN_DELETE_CHANGES
        )

    def revert_is_allowed(self, request, obj):
        return (
            REVERT_IS_ALLOWED(request, obj)
            if callable(REVERT_IS_ALLOWED)
            else REVERT_IS_ALLOWED
        )

    def get_readonly_fields(self, request, obj=None):
        fields = [f.name for f in obj._meta.fields if f.name != "revision"]
        if (
            CAN_CHANGE_CHANGES(request, obj)
            if callable(CAN_CHANGE_CHANGES)
            else CAN_CHANGE_CHANGES
        ):
            return fields
    ...
    def revert_view(self, request, object_id, extra_context=None):
        ...
        if request.method == "POST":
            ...
            try:
                obj.revert()
                messages.success(
                    request, "Changes of %s was reverted" % obj.object_repr
                )
    ...
    def get_urls(self):
        ...
        urls.insert(
            0,
            re_path(r"^(.+)/revert/$", wrap(self.revert_view), name="revert_changes"),
        )
        return urls
...
class RevisionAdmin(FastObjectsCountAdminModel):
    ...
    def has_delete_permission(self, request, obj=None):
        return (
            CAN_DELETE_REVISION(request, obj)
            if callable(CAN_DELETE_REVISION)
            else CAN_DELETE_REVISION
        )
    ...
    def changes(self, obj):
        ...
        return format_html_join(
            ", ",
            '<a href="{}">{}</a>',
            ((i.get_admin_url(), i.id) for i in obj.change_set.all()),
        )
    ...
    def get_inline_formsets(self, request, formsets, inline_instances, obj=None):
        ...
        return super(RevisionAdmin, self).get_inline_formsets(
    ...
    def get_urls(self):
        ...
        urls.insert(
            0,
            re_path(r"^(.+)/revert/$", wrap(self.revert_view), name="revert_revision"),
        )
        return urls
        ...
        if request.method == "POST":
            try:
                with transaction.atomic():
                    obj.revert()
                    messages.success(
                        request, "Changes of %s was reverted" % force_str(obj)
                    )
